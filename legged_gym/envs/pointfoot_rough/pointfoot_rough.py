...
class BipedPF(BaseTask):
    ...
    def compute_group_observations(self):
        ...
        critic_obs_buf = torch.cat((  # in total: 3 + 2 + 6 + 1 + 1 + 3 + 6 + 6 + 6 + 1 + 1 = 36
            ...
            self.p_gains,  # 6
            self.d_gains,  # 6
            self.torques_scale,  # 6
            self.restitution_coef.unsqueeze(-1),  # 1
            self.friction_coef.unsqueeze(-1),  # 1
            self.obs_buf), dim=-1)
        

        if self.cfg.terrain.critic_measure_heights:
            heights = torch.clip(self.root_states[:, 2].unsqueeze(1) - 0.6 - self.measured_heights, -1, 1.) * self.obs_scales.height_measurements
    ...
    def _reward_tracking_ang_vel(self):
        ...
        return torch.exp(-ang_vel_error / self.cfg.rewards.ang_tracking_sigma) * torch.clamp(-self.projected_gravity[:, 2], 0, 1)
    ...
    def _reward_tracking_contacts_shaped_force(self):
        ...
        return torch.clamp(-self.projected_gravity[:, 2], 0, 1) * reward / len(self.feet_indices)
